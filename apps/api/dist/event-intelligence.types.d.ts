...
export type EventStatus = 'interpreted' | 'needs_resolution' | 'reviewed' | 'approval_pending' | 'approved' | 'applied' | 'rolled_back' | 'rejected';
...
export interface CausalGraph {
    ...
    edges: Array<{
        from: string;
        to: string;
        relation: 'affects' | 'causes' | 'may_cause';
        confidence: number;
    }>;
    max_depth: 3;
...
export type EventAuditIssueCode = 'empty_chain' | 'invalid_records' | 'hash_gaps' | 'forks' | 'orphan_records' | 'sequence_gaps' | 'incomplete_event_chain';
...
export interface IntelligenceEvent {
    ...
    occurred_at: {
        value: string;
        precision: 'exact' | 'day' | 'relative' | 'unknown';
        confidence: number;
    };
    recorded_at: string;
    ...
    evidence: Array<{
        evidence_id: string;
        kind: 'user_statement' | 'synthetic_graph_fact' | 'conflict';
        summary: string;
        confidence: number;
    }>;
    attachments: Array<{
    ...
    historical_references: Array<{
        event_id: string;
        relation: 'conflicts_with' | 'supersedes' | 'reverses';
    }>;
    impacts: EventImpact[];
