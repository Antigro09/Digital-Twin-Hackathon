...
exports.EventIntelligenceService = exports.EVENT_CHANGED_SCHEMA_HASH = exports.EVENT_CHANGED_SCHEMA_ID = void 0;
...
const MODEL_VERSION = 'event-extractor-rules/1.0.0';
...
function loadEventChangedSchema() {
    const candidates = [
        (0, node_path_1.resolve)(process.cwd(), 'docs/enterprise-digital-twin/contracts/schemas', EVENT_CHANGED_SCHEMA_FILE),
        (0, node_path_1.resolve)(process.cwd(), '../../docs/enterprise-digital-twin/contracts/schemas', EVENT_CHANGED_SCHEMA_FILE),
        (0, node_path_1.resolve)(__dirname, '../../../docs/enterprise-digital-twin/contracts/schemas', EVENT_CHANGED_SCHEMA_FILE),
    ];
    const path = candidates.find((candidate) => (0, node_fs_1.existsSync)(candidate));
    if (!path)
        throw new Error(`Event Intelligence CloudEvent schema ${EVENT_CHANGED_SCHEMA_FILE} was not found.`);
    const schema = JSON.parse((0, node_fs_1.readFileSync)(path, 'utf8'));
...
exports.EVENT_CHANGED_SCHEMA_ID = EVENT_CHANGED_SCHEMA.$id;
exports.EVENT_CHANGED_SCHEMA_HASH = (0, domain_1.sha256)(EVENT_CHANGED_SCHEMA);
const taxonomySeed = [
...
const TAXONOMY = taxonomySeed.map(([code, category, label, defaultMode, sensitive, examples]) => ({
    code,
    category,
    label,
...
let EventIntelligenceService = class EventIntelligenceService {
    database;
    projection;
    events = new Map();
    approvals = new Map();
    ...
    constructor(database, projection) {
    ...
    async onModuleInit() {
        ...
        if (!this.database.enabled)
            return;
        for (const tenantId of [domain_1.ASTER_TENANT_ID, domain_1.BEACON_TENANT_ID]) {
            const projectionId = (0, domain_1.stableUuid)(`event-projection:${tenantId}`);
            const persistedProjection = await this.database.get(tenantId, 'event_projection_snapshot', projectionId);
            if (persistedProjection)
                this.projection.hydrate(tenantId, persistedProjection);
            ...
            const events = await this.database.list(tenantId, 'intelligence_event');
            ...
            const receipts = await this.database.list(tenantId, 'event_action_receipt');
            ...
            const applications = await this.database.list(tenantId, 'event_application');
            for (const application of applications) {
                ...
                if (!event
                    ...
                    || (application.before_snapshot !== null && (application.before_snapshot.version !== application.before_graph_version
                        || application.before_snapshot.state_hash !== application.before_state_hash))
                    || (application.after_snapshot !== null && (application.after_snapshot.version !== application.after_graph_version
                        || application.after_snapshot.state_hash !== application.after_state_hash)))
                    continue;
                this.applications.set(application.event_id, application);
            }
            const branches = await this.database.list(tenantId, 'event_branch');
            for (const branch of branches) {
                if (branch.tenant_id !== tenantId || !this.validSnapshotBinding(branch.base_graph_version, branch.base_state_hash) || !this.validHash(branch.state_hash))
                    continue;
                this.branches.set(branch.branch_id, branch);
            }
            const timeline = await this.database.list(tenantId, 'event_timeline_entry');
            const validTimeline = timeline.filter((entry) => (entry.tenant_id === tenantId
            ...
            if (validTimeline.length)
                this.timelines.set(tenantId, validTimeline.sort((left, right) => left.sequence - right.sequence));
            const audits = await this.database.list(tenantId, 'event_audit_evidence');
            ...
            const idempotency = await this.database.list(tenantId, 'event_idempotency');
            for (const record of idempotency) {
                if (typeof record.scope !== 'string' || !record.scope.startsWith(`${tenantId}:`))
                    continue;
    ...
    async interpret(ctx, body, idempotencyKey) {
        ...
        if (text.length < 3 || text.length > MAX_INPUT_CHARACTERS) {
            throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'invalid_event_text', `text must contain 3 to ${MAX_INPUT_CHARACTERS} characters.`);
        }
        const requestedMode = body.requested_mode ?? 'auto';
        if (!['auto', 'reality', 'scenario'].includes(String(requestedMode))) {
            throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'invalid_event_mode', 'requested_mode must be auto, reality, or scenario.');
        }
        ...
        if (suppliedOccurredAt !== undefined && (typeof suppliedOccurredAt !== 'string' || !this.validIso(suppliedOccurredAt))) {
            throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'invalid_event_timestamp', 'occurred_at must be an ISO-8601 timestamp.');
        }
        ...
        const interpretationId = (0, domain_1.newId)();
        ...
        for (let index = 0; index < classifications.length; index += 1) {
            ...
            const record = await this.buildEvent(ctx, interpretationId, classified.statement, classified.code, requestedMode, typeof suppliedOccurredAt === 'string' ? suppliedOccurredAt : undefined, safety, index);
    ...
    listEvents(ctx, pageSizeInput, cursorInput) {
        this.assertRead(ctx);
        const pageSize = pageSizeInput === undefined ? 50 : Number(pageSizeInput);
        if (!Number.isInteger(pageSize) || pageSize < 1 || pageSize > 100) {
            throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'invalid_page_size', 'page_size must be an integer from 1 to 100.');
        }
        ...
        if (cursorInput) {
            try {
                const cursor = JSON.parse(Buffer.from(cursorInput, 'base64url').toString('utf8'));
                const fingerprint = (0, domain_1.sha256)({ tenant_id: ctx.tenantId, membership_id: ctx.membershipId, policy_version: ctx.policyVersion }).slice(0, 24);
                if (cursor.version !== 1 || cursor.authorization_fingerprint !== fingerprint || typeof cursor.last_event_id !== 'string')
                ...
                throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'invalid_page_cursor', 'page_cursor is invalid for the active tenant and authorization context.');
            }
        }
        ...
        const nextCursor = hasMore && last
            ? Buffer.from(JSON.stringify({
                version: 1,
                authorization_fingerprint: (0, domain_1.sha256)({ tenant_id: ctx.tenantId, membership_id: ctx.membershipId, policy_version: ctx.policyVersion }).slice(0, 24),
    ...
    async review(ctx, eventId, body, ifMatch, idempotencyKey) {
        ...
        if (['applied', 'rolled_back', 'rejected'].includes(currentEvent.status)) {
            throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'event_terminal', `Event status ${currentEvent.status} is terminal; create a superseding event instead of rewriting its application history.`);
        }
        ...
        if (event.safety.quarantined) {
            throw new problem_1.ProblemException(common_1.HttpStatus.UNPROCESSABLE_ENTITY, 'event_quarantined', 'Prompt-like instructions are quarantined and cannot become graph changes.');
        }
        const verification = body.verification_status;
        if (!['unverified', 'confirmed', 'rejected'].includes(String(verification))) {
            throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'invalid_verification_status', 'verification_status must be unverified, confirmed, or rejected.');
        }
        const targetMode = body.target_mode;
        if (!['reality', 'scenario'].includes(String(targetMode))) {
            throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'invalid_event_mode', 'target_mode must be reality or scenario.');
        }
        if (targetMode === 'reality' && verification !== 'confirmed') {
            throw new problem_1.ProblemException(common_1.HttpStatus.UNPROCESSABLE_ENTITY, 'reality_verification_required', 'Reality updates require confirmed human verification; an unverified hypothesis must remain a scenario.');
        }
        const notes = typeof body.notes === 'string' ? body.notes.trim() : '';
        if (notes.length < 4 || notes.length > 1_000) {
            throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'review_notes_required', 'notes must contain 4 to 1000 characters.');
        }
        ...
        for (const resolution of event.entity_resolutions) {
            ...
            if (!decision) {
                if (resolution.candidates.length) {
                    throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'entity_confirmation_required', `A resolution decision is required for ${resolution.mention}.`);
                }
                continue;
            }
            if (decision.selected_entity_id !== null && !resolution.candidates.some((candidate) => candidate.entity_id === decision.selected_entity_id)) {
                throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'invalid_entity_candidate', 'A selected entity is not an authorized candidate for this event.');
            }
        ...
        event.mode = targetMode;
        if (verification === 'rejected') {
            event.status = 'rejected';
            event.gate = this.rejectedGate('A reviewer rejected the factual claim.');
        }
        else {
            const taxonomy = TAXONOMY.find((item) => item.code === event.event_type.code) ?? TAXONOMY.at(-1);
            event.impacts = this.impactsFor(ctx.tenantId, event.event_id, taxonomy, event.entity_resolutions, event.confidence.score, event.statement, event.evidence.map((item) => item.evidence_id));
            event.causal_graph = this.causalGraph(event.event_id, event.event_type.label, event.impacts, event.statement);
            event.gate = this.gateFor(event, targetMode);
            if (targetMode === 'reality' && !event.gate.live_mutation_allowed) {
                throw new problem_1.ProblemException(common_1.HttpStatus.UNPROCESSABLE_ENTITY, 'reality_gate_failed', `Reality update is blocked: ${event.gate.blockers.join('; ')}.`);
            }
            event.status = 'reviewed';
        }
        ...
        event.version += 1;
        event.reviewed_payload_hash = (0, domain_1.sha256)({
    ...
    async requestApproval(ctx, eventId, body, ifMatch, idempotencyKey) {
        ...
        if (event.status !== 'reviewed' || !event.reviewed_payload_hash) {
            throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'event_not_reviewed', 'Review and confirm the exact interpretation before requesting approval.');
        }
        if (body.reviewed_payload_hash !== event.reviewed_payload_hash)
            this.payloadMismatch();
        const reason = typeof body.reason === 'string' ? body.reason.trim() : '';
        if (reason.length < 8 || reason.length > 500) {
            throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'approval_reason_required', 'reason must contain 8 to 500 characters.');
        }
        const approvalId = (0, domain_1.newId)();
        const requiredRoles = event.mode === 'reality' ? ['operations', 'security'] : [];
        ...
        const approval = {
            ...
            expires_at: (0, domain_1.addSeconds)((0, domain_1.nowIso)(), 900),
    ...
    async decideApproval(ctx, approvalId, body, idempotencyKey) {
        const approval = this.approvals.get(approvalId);
        if (!approval || approval.tenant_id !== ctx.tenantId)
            throw this.notFound();
        ...
        if (approval.approval_kind !== 'dual_human' || approval.status !== 'pending') {
            throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'approval_not_open', 'This approval is not open for human decisions.');
        }
        if (this.isExpired(approval.expires_at)) {
            ...
            await this.database.put(ctx.tenantId, 'event_approval', approvalId, approval);
            await this.database.put(ctx.tenantId, 'intelligence_event', event.event_id, event);
            throw new problem_1.ProblemException(common_1.HttpStatus.GONE, 'approval_expired', 'The exact-payload approval expired.');
        }
        if (body.payload_hash !== approval.payload_hash)
            this.payloadMismatch();
        if (!['approve', 'deny'].includes(String(body.decision))) {
            throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'invalid_approval_decision', 'decision must be approve or deny.');
        }
        if (!approval.required_roles.includes(role)) {
            throw new problem_1.ProblemException(common_1.HttpStatus.FORBIDDEN, 'approval_role_not_required', 'This approval does not require the actor role.');
        }
        if (approval.decisions.some((decision) => decision.actor_id === ctx.actor.actor_id || decision.role === role)) {
            throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'duplicate_approval_slot', 'Each approval slot requires one distinct authenticated approver.');
        }
        const decision = {
            decision_id: (0, domain_1.newId)(),
            actor_id: ctx.actor.actor_id,
            role,
            decision: body.decision,
            payload_hash: approval.payload_hash,
            decided_at: (0, domain_1.nowIso)(),
        };
        ...
        approval.audit_evidence = await this.appendAudit(ctx, `event.approval.${decision.decision}`, 'event_approval', approvalId, (0, domain_1.sha256)(decision));
        event.audit_evidence = approval.audit_evidence;
        this.idempotency.set(scope, { request_hash: requestHash, resource_id: approvalId });
        await this.database.put(ctx.tenantId, 'event_approval', approvalId, approval);
        await this.database.put(ctx.tenantId, 'intelligence_event', event.event_id, event);
    ...
    async apply(ctx, eventId, body, ifMatch, idempotencyKey) {
        const event = this.eventForContext(ctx, eventId);
        this.assertCreatorOrCreate(ctx, event);
        ...
        if (event.status !== 'approved' || !event.reviewed_payload_hash) {
            throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'approval_incomplete', 'The exact reviewed payload must be fully approved before apply.');
        }
        if (body.reviewed_payload_hash !== event.reviewed_payload_hash)
            this.payloadMismatch();
        const approval = this.approvals.get(String(body.approval_id));
        if (!approval || approval.tenant_id !== ctx.tenantId || approval.event_id !== event.event_id)
            throw this.notFound();
        if (approval.status !== 'approved' || approval.payload_hash !== event.reviewed_payload_hash) {
            throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'approval_incomplete', 'The approval is incomplete or bound to a different payload.');
        }
        ...
        if (event.version !== expectedApprovedEventVersion) {
            throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'approval_event_version_mismatch', 'The approved review is bound to a different event version.');
        }
        if (approval.graph_snapshot_version !== event.graph_snapshot_version
            || approval.graph_snapshot_hash !== event.graph_snapshot_hash) {
            throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'approval_snapshot_mismatch', 'The approval is bound to a different graph snapshot.');
        }
        if (this.isExpired(approval.expires_at))
            throw new problem_1.ProblemException(common_1.HttpStatus.GONE, 'approval_expired', 'The exact-payload approval expired.');
        ...
        let beforeStateHash;
        let afterStateHash;
        let graphVersionBefore;
        let graphVersionAfter;
        ...
        const proposedChanges = event.impacts
            ...
            .map((impact) => ({
            impact,
            change: { impact_id: impact.impact_id, ...structuredClone(impact.proposed_mutation) },
        }));
        ...
        if (event.mode === 'reality' && appliedChanges.length === 0) {
            throw new problem_1.ProblemException(common_1.HttpStatus.UNPROCESSABLE_ENTITY, 'no_live_mutation_eligible_changes', 'The reviewed event contains no allowlisted authoritative projection mutation; derived proposals may be evaluated only in a scenario branch.');
        }
        let nextBranch;
        if (event.mode === 'scenario') {
            ...
            if (currentProjection.version !== approval.graph_snapshot_version) {
                throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'projection_version_changed', 'The synthetic event projection version changed; refresh and review the scenario again.');
            }
            if (currentProjection.state_hash !== approval.graph_snapshot_hash) {
                throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'projection_hash_changed', 'The synthetic event projection state changed; refresh and review the scenario again.');
            }
            beforeSnapshot = currentProjection;
            branchId = (0, domain_1.stableUuid)(`event-branch:${ctx.tenantId}:${event.event_id}`);
            beforeStateHash = currentProjection.state_hash;
            afterStateHash = (0, domain_1.sha256)({
            ...
            let transition;
            try {
                transition = this.projection.applyExact(ctx.tenantId, approval.graph_snapshot_version, approval.graph_snapshot_hash, appliedChanges);
            }
            catch (error) {
            ...
            if (!nextBranch.event_ids.includes(event.event_id))
                nextBranch.event_ids.push(event.event_id);
        }
        const application = {
            event_id: event.event_id,
        ...
        const payloadHash = (0, domain_1.sha256)({ event_id: event.event_id, approval_id: approval.approval_id, reviewed_payload_hash: event.reviewed_payload_hash, appliedChanges });
        ...
        const timelineEntry = this.makeTimelineEntry(ctx.tenantId, nextEvent, beforeStateHash, afterStateHash, graphVersionBefore, graphVersionAfter, receiptId, 'event_applied');
        const idempotencyRecord = { scope, request_hash: requestHash, resource_id: receiptId };
        const outboxEventId = (0, domain_1.newId)();
        const outboxPayload = this.eventChangedEnvelope(ctx, nextEvent, receipt, outboxEventId);
        const mutationGuard = this.eventMutationGuard(ctx, `apply:${eventId}`, idempotencyKey, requestHash, receiptId, event, beforeSnapshot);
        try {
            const result = await this.database.commitEventMutation(ctx.tenantId, [
                ...
                { kind: 'event_idempotency', id: (0, domain_1.stableUuid)(`event-idempotency:${scope}`), payload: idempotencyRecord },
                ...(afterSnapshot ? [{ kind: 'event_projection_snapshot', id: (0, domain_1.stableUuid)(`event-projection:${ctx.tenantId}`), payload: afterSnapshot }] : []),
            ], audit, {
            ...
            if (result.replayed) {
                ...
                if (!prior) {
                    throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'idempotency_response_missing', 'The authoritative idempotent event receipt is unavailable.');
                }
    ...
    async rollback(ctx, eventId, body, ifMatch, idempotencyKey) {
        const event = this.eventForContext(ctx, eventId);
        ...
        if (event.status !== 'applied' || !event.applied_payload_hash) {
            throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'event_not_applied', 'Only an applied synthetic event can be rolled back.');
        }
        if (body.applied_payload_hash !== event.applied_payload_hash)
            this.payloadMismatch();
        const reason = typeof body.reason === 'string' ? body.reason.trim() : '';
        if (reason.length < 8 || reason.length > 500) {
            throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'rollback_reason_required', 'reason must contain 8 to 500 characters.');
        }
        const beforeVersion = event.version;
        const application = this.applications.get(event.event_id);
        if (!application)
            throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'projection_receipt_missing', 'The exact synthetic projection receipt required for rollback is unavailable.');
        ...
        let graphVersionBefore;
        let graphVersionAfter;
        let projectionBefore = null;
        let projectionAfter = null;
        let nextBranch = null;
        if (event.mode === 'scenario' && event.branch_id) {
            ...
            if (currentSnapshot.version !== application.after_graph_version
                || currentSnapshot.state_hash !== application.after_state_hash) {
                throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'projection_version_changed', 'The synthetic reality projection changed after this event; an exact rollback would overwrite later work.');
            }
            if (!application.before_snapshot) {
                throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'projection_receipt_missing', 'The exact before-state snapshot required for rollback is unavailable.');
            }
            let transition;
            try {
                transition = this.projection.rollbackExact(ctx.tenantId, currentSnapshot.version, currentSnapshot.state_hash, application.before_snapshot);
            }
            catch (error) {
                this.throwProjectionProblem(error);
            }
        ...
        nextEvent.version += 1;
        const payloadHash = (0, domain_1.sha256)({ applied_payload_hash: event.applied_payload_hash, inverseChanges, reason });
        const receiptId = (0, domain_1.newId)();
        const auditCount = (this.audits.get(ctx.tenantId) ?? []).length;
        ...
        nextEvent.audit_evidence = audit;
        this.refreshEtag(nextEvent);
        const timelineEntry = this.makeTimelineEntry(ctx.tenantId, nextEvent, application.after_state_hash, application.before_state_hash, graphVersionBefore, graphVersionAfter, receiptId, 'event_rolled_back');
        const idempotencyRecord = { scope, request_hash: requestHash, resource_id: receiptId };
        const outboxEventId = (0, domain_1.newId)();
        const outboxPayload = this.eventChangedEnvelope(ctx, nextEvent, receipt, outboxEventId);
        const mutationGuard = this.eventMutationGuard(ctx, `rollback:${eventId}`, idempotencyKey, requestHash, receiptId, event, projectionBefore);
        try {
            const result = await this.database.commitEventMutation(ctx.tenantId, [
                { kind: 'intelligence_event', id: nextEvent.event_id, payload: nextEvent },
                { kind: 'event_action_receipt', id: receiptId, payload: receipt },
                ...
                ...(projectionAfter ? [{ kind: 'event_projection_snapshot', id: (0, domain_1.stableUuid)(`event-projection:${ctx.tenantId}`), payload: projectionAfter }] : []),
            ], audit, {
                eventId: outboxEventId,
                eventType: 'com.enterprisedigitaltwin.event-intelligence.event-changed.v1',
                aggregateType: 'intelligence_event',
            ...
            if (result.replayed) {
                ...
                const prior = await this.reloadAuthoritativeMutation(ctx.tenantId, event.event_id, result.responseRef ?? receiptId, scope, requestHash);
                if (!prior) {
                    throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'idempotency_response_missing', 'The authoritative idempotent rollback receipt is unavailable.');
                }
                return { ...prior, replayed: true };
            }
            receipt.outbox_position = result.outboxPosition;
    ...
    replay(ctx, eventId) {
        ...
        if (application?.before_snapshot && application.after_snapshot) {
            ...
            for (const entityId of new Set([...beforeEntities.keys(), ...afterEntities.keys()])) {
                ...
                if ((0, domain_1.sha256)(before ?? null) === (0, domain_1.sha256)(after ?? null))
                    continue;
                entityChanges.push({
                    entity_id: entityId,
    ...
    compareBranches(ctx, body) {
        ...
        if (!left || !right || left.tenant_id !== ctx.tenantId || right.tenant_id !== ctx.tenantId)
            throw this.notFound();
    ...
    eventAudit(ctx, eventId) {
        ...
        const expectedIds = new Set([event.audit_evidence.audit_id]);
        for (const approval of this.approvals.values()) {
            if (approval.tenant_id === ctx.tenantId && approval.event_id === eventId)
                expectedIds.add(approval.audit_evidence.audit_id);
        }
        for (const receipt of this.receipts.values()) {
            if (receipt.tenant_id === ctx.tenantId && receipt.event_id === eventId)
                expectedIds.add(receipt.audit_evidence.audit_id);
        }
        const missingEventAuditIds = [...expectedIds].filter((auditId) => !canonicalIds.has(auditId)).sort();
        const issues = [...base.issues];
        if (items.length === 0 || missingEventAuditIds.length > 0)
            issues.push('incomplete_event_chain');
    ...
    async buildEvent(ctx, interpretationId, text, code, requestedMode, suppliedOccurredAt, safety, index) {
        const taxonomy = TAXONOMY.find((item) => item.code === code) ?? TAXONOMY.at(-1);
        const eventId = (0, domain_1.stableUuid)(`event:${ctx.tenantId}:${interpretationId}:${index}:${code}`);
        ...
        const evidence = [
            { evidence_id: (0, domain_1.stableUuid)(`event-evidence:${eventId}:statement`), kind: 'user_statement', summary: 'Unverified user-provided statement; treated as data, never as instructions.', confidence: confidence.score },
            ...resolutions.flatMap((resolution) => resolution.candidates.slice(0, 1).map((candidate) => ({
                evidence_id: (0, domain_1.stableUuid)(`event-evidence:${eventId}:${candidate.entity_id}`),
                kind: 'synthetic_graph_fact',
                summary: `Synthetic graph candidate ${candidate.display_name} matched by ${candidate.reasons.join(', ')}.`,
                confidence: candidate.confidence,
            }))),
            ...conflicts.map((conflict) => ({ evidence_id: (0, domain_1.stableUuid)(`event-evidence:${eventId}:conflict:${conflict.event_id}`), kind: 'conflict', summary: conflict.reason, confidence: 1 })),
        ];
        ...
        if (confidence.level !== 'confirmed')
            blockers.push(`confidence_level_${confidence.level}`);
        if (resolutions.some((resolution) => resolution.required_confirmation))
            blockers.push('entity_confirmation_required');
        if (conflicts.length)
            blockers.push('conflicting_event_requires_resolution');
        if (new Date(timing.value).getTime() < new Date('2015-01-01T00:00:00Z').getTime())
            blockers.push('timestamp_before_synthetic_company_history');
        if (safety.quarantined)
            blockers.push('untrusted_instruction_quarantined');
        ...
        const audit = await this.appendAudit(ctx, safety.quarantined ? 'event.interpret.quarantined' : 'event.interpret', 'event', eventId, (0, domain_1.sha256)({ text, code, mode }));
        const graphSnapshot = this.projection.snapshot(ctx.tenantId);
        const event = {
            event_id: eventId,
            interpretation_id: interpretationId,
            ...
            source: { kind: 'manual_natural_language', creator_id: ctx.actor.actor_id, source_text_hash: (0, domain_1.sha256)(text) },
            ...
            evidence,
            attachments: [],
            historical_references: conflicts.map((conflict) => ({ event_id: conflict.event_id, relation: 'conflicts_with' })),
            impacts,
            causal_graph: graph,
    ...
    classify(text) {
        ...
        for (const statement of segments) {
            ...
            for (const rule of RULES) {
                ...
                if (match)
                    results.push({ code: rule.code, matchIndex: Math.max(0, segmentIndex) + match.index, statement });
            }
        }
    ...
    confidenceFor(text, taxonomy, safety) {
        if (safety.quarantined)
            return { score: 0, level: 'rejected', rationale: ['Input was quarantined as instruction-like content.'] };
    ...
    initialMode(text, taxonomy, requested, level, occurredAt) {
        if (requested === 'scenario')
            return 'scenario';
        if (requested === 'reality')
            return level === 'confirmed' && new Date(occurredAt).getTime() <= new Date((0, domain_1.nowIso)()).getTime() ? 'reality' : 'scenario';
        if (taxonomy.default_mode === 'scenario' || level !== 'confirmed' || /\bwhat if\b/i.test(text))
            return 'scenario';
        return 'reality';
    }
    resolveEntities(tenantId, text, category) {
        ...
        const candidates = fixtures.map((entity) => {
            ...
            if (category === 'people' && entity.entity_type !== 'person' && entity.entity_type !== 'team')
                confidence *= 0.5;
            if (category === 'project' && entity.entity_type !== 'project')
                confidence *= 0.65;
            if (category === 'technology' && !['service', 'database'].includes(entity.entity_type))
                confidence *= 0.45;
            if (category === 'business' && !['customer', 'vendor'].includes(entity.entity_type))
                confidence *= 0.45;
            if (category === 'operations' && !['asset', 'vendor', 'service'].includes(entity.entity_type))
                confidence *= 0.45;
        ...
        if (!mention && candidates.length)
            mention = candidates[0].display_name;
        if (!mention)
            mention = category === 'external' ? 'external event' : 'unknown entity';
    ...
    entities() {
        ...
        return [domain_1.ASTER_TENANT_ID, domain_1.BEACON_TENANT_ID].flatMap((tenantId) => this.projection.catalog(tenantId)
            .filter((entity) => supported.has(entity.entity_type))
            .map((entity) => ({
            ...entity,
    ...
    impactsFor(tenantId, eventId, taxonomy, resolutions, confidence, text, evidenceIds) {
        ...
        const activeRelationship = (type) => projection.relationships.find((relationship) => (relationship.type === type
            && relationship.from_entity_id === entity?.entity_id
            && relationship.state === 'active'));
        const displayNameFor = (entityId, fallback) => (projection.entities.find((candidate) => candidate.entity_id === entityId)?.display_name ?? fallback);
        const affected = (fallbackType, fallbackName) => ({
            entity_id: entity?.entity_id ?? null,
            entity_type: entity?.entity_type ?? fallbackType,
            display_name: entity?.display_name ?? fallbackName,
        });
        const impacts = [];
        const add = (depth, kind, name, severity, explanation, action, mutation, causedBy, relationship = null, horizon = depth === 0 ? 'immediate' : depth === 1 ? 'days' : depth === 2 ? 'weeks' : 'months') => {
            const id = (0, domain_1.stableUuid)(`impact:${eventId}:${impacts.length}:${name}`);
    ...
    causalGraph(eventId, label, impacts, text) {
        const nodes = [{ id: eventId, kind: 'event', label, depth: 0 }];
        ...
        const edges = impacts.map((impact) => ({
            ...
            to: impact.impact_id,
            relation: impact.confidence >= 0.8 ? 'causes' : 'may_cause',
    ...
    gateFor(event, mode) {
        ...
        const blockers = [];
        if (event.confidence.level !== 'confirmed')
            blockers.push(`confidence_level_${event.confidence.level}`);
        if (event.verification_status !== 'confirmed')
            blockers.push('human_verification_required');
        if (event.entity_resolutions.some((resolution) => resolution.required_confirmation || resolution.selected_entity_id === null))
            blockers.push('entity_confirmation_required');
        if (event.conflicts.length)
            blockers.push('conflicting_event_requires_resolution');
        if (!event.impacts.some((impact) => impact.live_mutation_eligible && impact.proposed_mutation !== null))
            blockers.push('no_live_mutation_eligible_changes');
        return {
    ...
    findConflicts(tenantId, code, resolutions) {
        ...
        return [...this.events.values()].filter((prior) => {
            if (prior.tenant_id !== tenantId || prior.status === 'rejected' || prior.status === 'rolled_back')
                return false;
    ...
    extractTiming(text, supplied) {
        if (supplied)
            return { value: new Date(supplied).toISOString(), precision: 'exact', confidence: 1 };
        const now = new Date((0, domain_1.nowIso)());
        if (/\byesterday\b/i.test(text)) {
            now.setUTCDate(now.getUTCDate() - 1);
            return { value: now.toISOString(), precision: 'relative', confidence: 0.9 };
        }
        if (/\btoday\b/i.test(text))
            return { value: now.toISOString(), precision: 'day', confidence: 0.9 };
    ...
    parseResolutionDecisions(value) {
        if (!Array.isArray(value))
            throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'entity_resolutions_required', 'entity_resolutions must be an array.');
        return value.map((item) => {
            if (!item || typeof item !== 'object' || Array.isArray(item))
                throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'invalid_entity_resolution', 'Each entity resolution must be an object.');
            const record = item;
            ...
            if (typeof record.mention !== 'string' || (record.selected_entity_id !== null && typeof record.selected_entity_id !== 'string')) {
                throw new problem_1.ProblemException(common_1.HttpStatus.BAD_REQUEST, 'invalid_entity_resolution', 'Entity resolution requires mention and a candidate ID or null.');
            }
            return { mention: record.mention, selected_entity_id: record.selected_entity_id };
        });
    }
    ensureTenant(ctx) {
        const baselineId = this.baselineBranchId(ctx.tenantId);
        if (!this.branches.has(baselineId)) {
            ...
            this.timelines.set(ctx.tenantId, [{
                    timeline_entry_id: (0, domain_1.stableUuid)(`timeline-baseline:${ctx.tenantId}`),
                    tenant_id: ctx.tenantId,
                    event_id: (0, domain_1.stableUuid)(`baseline-event:${ctx.tenantId}`),
    ...
    makeTimelineEntry(tenantId, event, beforeHash, afterHash, graphVersionBefore, graphVersionAfter, receiptId, action) {
        const entries = this.timelines.get(tenantId) ?? [];
        return {
            timeline_entry_id: (0, domain_1.newId)(),
            tenant_id: tenantId,
            event_id: event.event_id,
    ...
    async persistTenantBaseline(tenantId) {
        const branch = this.branches.get(this.baselineBranchId(tenantId));
        const entry = this.timelines.get(tenantId)?.[0];
        if (branch)
            await this.database.put(tenantId, 'event_branch', branch.branch_id, branch);
        if (entry)
            await this.database.put(tenantId, 'event_timeline_entry', entry.timeline_entry_id, entry);
    }
    async persistIdempotency(tenantId, scope, record) {
        await this.database.put(tenantId, 'event_idempotency', (0, domain_1.stableUuid)(`event-idempotency:${scope}`), { scope, ...record });
    }
    throwProjectionProblem(error) {
        if (error instanceof event_projection_service_1.EventProjectionError) {
            throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, error.code, error.message);
        }
        throw error;
    }
    eventChangedEnvelope(ctx, event, receipt, outboxEventId) {
        ...
        return {
            ...
            dataschema: exports.EVENT_CHANGED_SCHEMA_ID,
            tenant_id: ctx.tenantId,
            ...
            traceparent: `00-${receipt.audit_evidence.trace_id}-${(0, domain_1.sha256)(outboxEventId).slice(0, 16)}-01`,
            schema_version: 1,
            schema_hash: exports.EVENT_CHANGED_SCHEMA_HASH,
    ...
    baselineBranchId(tenantId) {
        return (0, domain_1.stableUuid)(`event-baseline-branch:${tenantId}`);
    }
    eventForContext(ctx, eventId) {
        this.assertRead(ctx);
        const event = this.events.get(eventId);
        if (!event || event.tenant_id !== ctx.tenantId)
            throw this.notFound();
        return event;
    }
    assertRead(ctx) {
        ...
        if (!permitted || ctx.actor.actor_alias === 'usr_platform_operator' || ctx.actor.actor_alias === 'usr_aster_limited') {
            throw new problem_1.ProblemException(common_1.HttpStatus.FORBIDDEN, 'event_intelligence_read_denied', 'Event intelligence access is not authorized.');
        }
    }
    assertCreate(ctx) {
        this.assertRead(ctx);
        if (!ctx.actor.capabilities.includes('scenario.create')) {
            throw new problem_1.ProblemException(common_1.HttpStatus.FORBIDDEN, 'event_intelligence_create_denied', 'Creating event interpretations is not authorized.');
        }
    }
    assertCreatorOrCreate(ctx, event) {
        this.assertCreate(ctx);
        if (event.source.creator_id !== ctx.actor.actor_id) {
            throw new problem_1.ProblemException(common_1.HttpStatus.FORBIDDEN, 'event_intelligence_change_denied', 'Only the interpretation creator can advance this synthetic event workflow.');
        }
    }
    assertRollback(ctx) {
        this.assertRead(ctx);
        if (!ctx.actor.capabilities.includes('action.approve.operations') && !ctx.actor.capabilities.includes('action.approve.security')) {
            throw new problem_1.ProblemException(common_1.HttpStatus.FORBIDDEN, 'event_rollback_denied', 'Rollback requires an authenticated operations or security approver.');
        }
    }
    approverRole(ctx) {
        this.assertRead(ctx);
        if (ctx.actor.capabilities.includes('action.approve.operations'))
            return 'operations';
        if (ctx.actor.capabilities.includes('action.approve.security'))
            return 'security';
        throw new problem_1.ProblemException(common_1.HttpStatus.FORBIDDEN, 'event_approval_denied', 'A required event approval role is not authorized.');
    }
    assertExpectedVersion(event, value) {
        ...
        if (version !== event.version)
            throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'event_version_changed', 'The event changed; refresh before continuing.');
    }
    assertPrecondition(event, ifMatch) {
        if (!ifMatch)
            throw new problem_1.ProblemException(common_1.HttpStatus.PRECONDITION_REQUIRED, 'if_match_required', 'If-Match must bind the request to the current event version.');
        if (ifMatch !== event.etag)
            throw new problem_1.ProblemException(common_1.HttpStatus.PRECONDITION_FAILED, 'event_precondition_failed', 'The event ETag changed; refresh before continuing.');
    }
    refreshEtag(event) {
        event.etag = (0, domain_1.etag)((0, domain_1.sha256)({ event_id: event.event_id, version: event.version, status: event.status, reviewed_payload_hash: event.reviewed_payload_hash, applied_payload_hash: event.applied_payload_hash }));
    }
    ...
    async appendAudit(ctx, action, resourceType, resourceId, detailsHash, persist = true) {
        ...
        if (blockingHealthIssue || (current.length === 0 && tenantHasEvents)) {
            throw new problem_1.ProblemException(common_1.HttpStatus.SERVICE_UNAVAILABLE, 'audit_chain_unhealthy', 'The tenant audit chain is empty, incomplete, invalid, gapped, or forked; audited mutation is disabled until the chain is repaired.');
        }
        ...
        const audit = { ...base, event_hash: (0, domain_1.sha256)({ ...base, tenant_id: ctx.tenantId }) };
        current.push(audit);
        const assessment = this.assessAuditChain(ctx.tenantId, current);
        if (assessment.diagnostics.issues.length > 0) {
            current.pop();
            throw new problem_1.ProblemException(common_1.HttpStatus.SERVICE_UNAVAILABLE, 'audit_chain_unhealthy', 'The generated audit event did not extend one complete tenant chain.');
        }
    ...
    auditBelongsToEvent(tenantId, audit, eventId) {
        if (audit.resource_type === 'event_approval')
            return this.approvals.get(audit.resource_id)?.tenant_id === tenantId && this.approvals.get(audit.resource_id)?.event_id === eventId;
        if (audit.resource_type === 'event_receipt')
            return this.receipts.get(audit.resource_id)?.tenant_id === tenantId && this.receipts.get(audit.resource_id)?.event_id === eventId;
        return false;
    }
    restoreAuditPrefix(tenantId, length) {
    ...
    assessAuditChain(tenantId, records) {
        ...
        const forkPreviousHashes = [...children.entries()]
            ...
            .sort();
        const walk = (previousHash, seen) => {
            let best = [];
            for (const candidate of children.get(previousHash) ?? []) {
                if (seen.has(candidate.audit_id))
                    continue;
        ...
        const issues = [];
        if (records.length === 0)
            issues.push('empty_chain');
        if (invalidRecordIds.length > 0)
            issues.push('invalid_records');
        if (gapRecords.length > 0)
            issues.push('hash_gaps');
        if (forkPreviousHashes.length > 0)
            issues.push('forks');
        if (orphanRecords.length > 0)
            issues.push('orphan_records');
        if (sequenceGapRecords.length > 0)
            issues.push('sequence_gaps');
    ...
    validAuditEvidence(tenantId, record) {
        if (!record || typeof record !== 'object')
            return false;
        if (!this.validUuid(record.audit_id)
        ...
        return (0, domain_1.sha256)({ ...base, tenant_id: tenantId }) === record.event_hash;
    }
    eventMutationGuard(ctx, operation, idempotencyKey, requestHash, responseRef, event, projectionSnapshot) {
        ...
        if (this.database.enabled && projectionSnapshot) {
            expectedRecords.push({
                ...
                id: (0, domain_1.stableUuid)(`event-projection:${ctx.tenantId}`),
    ...
    async reloadAuthoritativeMutation(tenantId, eventId, responseRef, idempotencyScope, requestHash) {
        if (!this.database.enabled) {
        ...
        if (persistedProjection)
            this.projection.hydrate(tenantId, persistedProjection);
        const persistedEvent = await this.database.get(tenantId, 'intelligence_event', eventId);
        if (persistedEvent?.tenant_id === tenantId)
            this.events.set(eventId, persistedEvent);
        const application = await this.database.get(tenantId, 'event_application', eventId);
        if (application)
            this.applications.set(eventId, application);
        let receipt;
        if (responseRef) {
            receipt = await this.database.get(tenantId, 'event_action_receipt', responseRef);
            if (receipt?.tenant_id === tenantId && receipt.event_id === eventId) {
                this.receipts.set(receipt.receipt_id, receipt);
                const approval = await this.database.get(tenantId, 'event_approval', receipt.approval_id);
                if (approval?.tenant_id === tenantId && approval.event_id === eventId)
                    this.approvals.set(approval.approval_id, approval);
                if (receipt.branch_id) {
                    const branch = await this.database.get(tenantId, 'event_branch', receipt.branch_id);
    ...
    async refreshAuthoritativeAudits(tenantId) {
        if (!this.database.enabled)
            return;
        const records = await this.database.list(tenantId, 'event_audit_evidence');
        const assessment = this.assessAuditChain(tenantId, records);
        this.audits.set(tenantId, assessment.canonical);
        this.auditHealth.set(tenantId, assessment.diagnostics);
    }
    throwDatabaseMutationProblem(error) {
        const status = error.code === 'event_precondition_failed'
            ? common_1.HttpStatus.PRECONDITION_FAILED
            : common_1.HttpStatus.CONFLICT;
        throw new problem_1.ProblemException(status, error.code, error.message);
    }
    scope(ctx, action, key) {
    ...
    validSnapshotBinding(version, stateHash) {
    ...
    payloadMismatch() {
        throw new problem_1.ProblemException(common_1.HttpStatus.CONFLICT, 'event_payload_mismatch', 'The payload hash does not match the reviewed and approved event.');
    }
    idempotencyConflict() {
...
exports.EventIntelligenceService = EventIntelligenceService = __decorate([
